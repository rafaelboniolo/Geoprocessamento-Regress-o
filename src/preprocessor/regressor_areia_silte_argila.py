...
def run():
    ...
    for i in range(0, len(data['areia_xxx_xxx'])):
        
        if '-' in str(data['areia_xxx_xxx'][i]):
            continue
        
        areia                    = float(str(data['areia_xxx_xxx'][i]).replace(',','.')) 
        silte                    = float(str(data['silte_xxx_xxx'][i]).replace(',','.')) 
    ...
    for i in range(0, len(data['areia_xxx_xxx'])):
        ...
        if '-' not in str(data['areia_xxx_xxx'][i]):
            continue
